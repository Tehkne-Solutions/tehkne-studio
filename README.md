...
- **Multi-turn Rotary Kinematics**: explicit signed revolution memory per rotary `connectedTo` relationship, composed with the S2.20 principal relative angle and S2.21 target delta to expose a continuous unwrapped angle across multiple turns.
...
The S2.20 principal relative angle remains **derived evidence**, reconstructed from persisted `inventionSpatial` transforms. S2.21 preserves absolute principal positioning through `principal-shortest` and does not persist the requested target as mechanical state.

S2.22 adds only the historical information that principal transforms cannot reconstruct after wrapping: an integer revolution count attached to the existing rotary relationship identity. The continuous angle is reconstructed as `revolutions × 2π + principalAngle`. Both `JOINT ±` and `SET ANGLE` advance the same history from the signed geometric delta actually executed, so principal target positioning remains compatible with multiple revolutions.

The S2.22 `inventionRotaryKinematics` extension is signed `Tehkné Solutions`, versioned and persisted in the same session snapshot. It accepts only authoritative `mechanical.rotary-shaft` `connectedTo` relationships, omits zero-revolution records, prunes disconnected relationships and rejects non-segmented commands larger than π radians.

Physics and simulation remain fail-closed. S2.22 does **not** claim RPM, angular velocity, angular acceleration or torque dynamics. Those require later explicitly modeled time/state/solver evidence.
...
The cumulative S2.22 CI preserves S2.14 Socket-Aware Wiring → S2.15 Direct Socket Wiring → S2.16 Mechanical Assembly → S2.17 Rigid Assembly Rotation → S2.18 Axial Joint Alignment → S2.19 Rotary Joint DOF → S2.20 Rotary Joint Relative Angle → S2.21 Rotary Joint Target Angle before validating S2.22 Multi-turn Rotary Kinematics, followed by the complete browser smoke and AF-001I deterministic evidence.
