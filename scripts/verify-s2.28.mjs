...
for (const token of [
  ...
  if (!rateMath.includes(token)) throw new Error(`S2.28 pure segment-rate math missing: ${token}`);
}
for (const forbidden of ["Date", "performance", "issuedAt", "occurredAt", "timestamp", "setTimeout", "requestAnimationFrame", "torque", "acceleration", "inertia"]) {
  if (rateMath.includes(forbidden)) throw new Error(`S2.28 segment-rate math must not infer time/dynamics: ${forbidden}`);
}
...
for (const token of [
  ...
  if (!runtime.includes(token)) throw new Error(`S2.28 command duration/rate evidence missing: ${token}`);
}
for (const forbidden of [
  ...
  if (runtime.includes(forbidden)) throw new Error(`S2.28 must avoid wall-clock inference/parallel dynamics state: ${forbidden}`);
}
const applyStart = runtime.indexOf("  #apply(");
const applyEnd = runtime.indexOf("  #recordEvidence(", applyStart);
if (applyStart < 0 || applyEnd < 0) throw new Error("S2.28 mechanical apply body missing");
...
if (limitCheck < 0 || rateDerivation < 0 || planner < 0 || transform < 0 || !(limitCheck < rateDerivation && rateDerivation < planner && planner < transform)) {
  throw new Error("S2.28 must validate S2.26 travel before rate evidence, planner and spatial mutation");
}
...
if (!movementFold.includes("MechanicalRotaryContinuousTargetExecuted")) throw new Error("S2.28 rate fold must include continuous-target motion evidence");
for (const forbidden of ["MechanicalRotaryTravelLimitsSet", "MechanicalRotaryTravelLimitsCleared", "MechanicalRotaryHomeSet", "MechanicalRotaryHomeRequested", "MechanicalRotaryHomeCleared"]) {
  if (movementFold.includes(forbidden)) throw new Error(`S2.28 non-motion audit event must not become rate evidence: ${forbidden}`);
}
...
for (const token of [
  ...
  if (!homeRuntime.includes(token)) throw new Error(`S2.28 GO HOME duration composition missing: ${token}`);
}
for (const forbidden of ["Date.parse(", "performance.now(", "rpmSolver", "rateByProject", "homeRateMap"]) {
  if (homeRuntime.includes(forbidden)) throw new Error(`S2.28 HOME orchestration must not infer/store rate independently: ${forbidden}`);
}
...
for (const token of [
  ...
  if (!control.includes(token)) throw new Error(`S2.28 UI segment-rate projection missing: ${token}`);
}
for (const forbidden of ["Date.now", "performance.now", "setInterval(", "requestAnimationFrame(", "torqueNm", "accelerationRad", "rateByProject"]) {
  if (control.includes(forbidden)) throw new Error(`S2.28 UI must not own a physical clock/dynamics solver: ${forbidden}`);
}

const domain = await readFile("tests/domain/invention-rotary-segment-rate.test.mjs", "utf8");
for (const token of [
  "S2.28 derives deterministic segment-average angular rate and RPM only from explicit duration",
  ...
  "latest untimed motion makes rate unresolved while travel and HOME authoring do not replace motion evidence",
  ...
  "rate must be sourced from the canonical movement command, not the HOME orchestration command",
...
for (const token of [
  "S2.28 derives explicit segment-average rotary rate across step targets limits HOME and restore",
  ...
  'data-rpm", "-5.000"',
  "GO HOME",
  'data-rate-command-id", "mechanical-cmd-7"',
  "Guardar 3D",
  "page.reload",
  'data-command-id", "mechanical-cmd-8"',
  "RATE UNRESOLVED"
]) {
...
if (!readme.includes("does **not** claim instantaneous angular velocity, acceleration, torque or time integration")) {
  throw new Error("S2.28 README must preserve the no-instantaneous-dynamics claim boundary");
}
...
for (const token of [
  "S2.28 Rotary Segment Rate Evidence Gate",
  "S2.27 rotary home position contract",
  "S2.28 rotary segment rate evidence contract",
  "npm run verify:s2.28",
  "S2.27 rotary home position browser contract",
  "S2.28 rotary segment rate evidence browser contract",
  "tests/browser/rotary-segment-rate.spec.ts",
  "s2-28-browser-failure"
]) {
  if (!workflow.includes(token)) throw new Error(`S2.28 CI promotion contract missing: ${token}`);
}
if (workflow.includes("contents: write")) throw new Error("S2.28 CI must remain read-only");

console.log("S2.28 Rotary Segment Rate Evidence PASS · explicit duration → deterministic segment-average rad/s + RPM for step/principal/continuous/GO HOME movement + S2.26 travel-limit precedence + S2.27 HOME orchestration preserved + persisted session-event evidence + untimed fail-closed + no wall-clock inference/no torque dynamics + Tehkné Solutions");
