...
for (const [portId, direction, z] of [["axis-in", "in", -0.0175], ["axis-out", "out", 0.0175]]) {
  ...
  if (!anchor || anchor.position?.[2] !== z) throw new Error(`S2.33 socket position mismatch: ${portId}`);
}
for (const claim of ["torqueCapacity", "maxRpm", "misalignmentCapacity", "stiffness", "damping", "manufacturingCertification"]) {
  if (coupler.metadata?.physicalClaims?.[claim] !== false) throw new Error(`S2.33 unsupported AF-002 physical claim must remain false: ${claim}`);
}
...
console.log("S2.33 AF-002 Dual-Shaft Assembly PASS · engineering-reference component + two canonical rotary ports + motor→coupler→wheel connectedTo topology + explicit proxy pending runtime GLB + no torque/RPM fiction + Tehkné Solutions");
