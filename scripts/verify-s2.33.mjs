...
for (const path of [
  ...
  "docs/platform/s2-33-waypoint-plan-execution-attestation.md",
  ".github/workflows/s2-33.yml"
...
for (const token of [
  ...
  "MechanicalRotaryWaypointPlanAttestationSegment",
  "runSequenceAttested(",
  "attestations(",
  "lastAttestation(",
  ...
  "mechanical-sequence-attestation-cmd-",
  "MECHANICAL_COMMAND_SIGNATURE"
]) if (!source.includes(token)) throw new Error(`S2.33 attestation contract missing: ${token}`);
...
]) if (source.includes(forbidden)) throw new Error(`S2.33 must reuse canonical S2.32 evidence and avoid parallel state/time/dynamics: ${forbidden}`);

const executeStart = source.indexOf("  async #executeAttestedRun(");
const executeEnd = source.indexOf("  #buildAttestation(", executeStart);
if (executeStart < 0 || executeEnd < 0) throw new Error("S2.33 attested run body missing");
...
if (plan < 0 || run < 0 || evidence < 0 || build < 0 || record < 0 || !(plan < run && run < evidence && evidence < build && build < record)) {
  throw new Error("S2.33 must capture plan, run canonically, resolve S2.32 evidence, verify, then persist attestation");
}
...
for (const token of ["rotaryWaypointExecutionEvidence(", "MechanicalRotaryWaypointSequenceRequested", 'derivedFrom: "session-events"', "MECHANICAL_COMMAND_SIGNATURE"]) {
  if (!s232.includes(token)) throw new Error(`S2.33 must preserve canonical S2.32 evidence authority: ${token}`);
}
for (const forbidden of ["events.record(", "commands.dispatch", "graph.replaceRelationship", "setContinuousTarget("]) {
  if (s232.includes(forbidden)) throw new Error(`S2.32 evidence must remain read-only under S2.33: ${forbidden}`);
}
...
for (const token of ["planSequence(relationshipId: string, name: string)", "this.#assertPlanAdmissible(plan)", "this.mechanical.setContinuousTarget(", "MechanicalRotaryWaypointSequenceRequested"]) {
  if (!sequenceRuntime.includes(token)) throw new Error(`S2.33 must preserve canonical S2.31/S2.30 run path: ${token}`);
}

const control = await readFile("apps/studio-web/components/RotaryWaypointSequenceControls.tsx", "utf8");
for (const token of [
  "mechanicalRotaryWaypointPlanAttestationRuntimeFor(spatial)",
  "attestationRuntime.runSequenceAttested",
  "attestationRuntime.lastAttestation",
  "PLAN-EXECUTION ATTESTATION NÃO DISPONÍVEL",
  "ATTESTED",
  "Rotary waypoint plan execution attestation summary",
  "Rotary waypoint plan execution attestation segments",
  "data-sequence-attestation-status",
  "data-sequence-attestation-command-id",
  "data-sequence-attestation-run-id",
  "data-sequence-attestation-derived-from",
  "data-sequence-attestation-plan-delta-rad",
  "data-sequence-attestation-actual-delta-rad",
  "data-sequence-attestation-match",
...
]) if (!control.includes(token)) throw new Error(`S2.33 UI attestation projection missing: ${token}`);
for (const forbidden of ["localStorage", "attestationMap", "setTimeout(", "setInterval(", "requestAnimationFrame(", "Date.now", "performance.now", "timelineState"]) {
  if (control.includes(forbidden)) throw new Error(`S2.33 UI attestation must not own persisted/time truth: ${forbidden}`);
}
...
for (const token of [
  ...
  "mechanical-sequence-attestation-cmd-1",
  "consumed-plan+s2.32-execution-evidence"
]) if (!domain.includes(token)) throw new Error(`S2.33 domain evidence missing: ${token}`);
...
for (const token of [
  ...
  'data-sequence-attestation-status", "verified"',
  'data-sequence-attestation-command-id", "mechanical-sequence-attestation-cmd-1"',
  ...
  'data-sequence-attestation-plan-delta-rad", "6.283"',
  'data-sequence-attestation-actual-delta-rad", "6.283"',
  "S2.32 actual",
  "120.0°",
  "90.0°",
  "Guardar 3D",
  "page.reload"
]) if (!browser.includes(token)) throw new Error(`S2.33 browser evidence missing: ${token}`);

const doc = await readFile("docs/platform/s2-33-waypoint-plan-execution-attestation.md", "utf8");
for (const token of ["S2.33", "Rotary Waypoint Plan-Execution Attestation", "S2.32", "consumed-plan+s2.32-execution-evidence", "session.events", "HERO_CANDIDATE", "AF-001L", "Tehkné Solutions"]) {
  if (!doc.includes(token)) throw new Error(`S2.33 candidate documentation missing: ${token}`);
}

const pkg = JSON.parse(await readFile("package.json", "utf8"));
if (pkg.scripts?.["verify:s2.33"] !== "node scripts/verify-s2.33.mjs") throw new Error("S2.33 package verification script missing");
const workflow = await readFile(".github/workflows/s2-33.yml", "utf8");
for (const token of [
  "S2.33 Rotary Waypoint Plan-Execution Attestation Gate",
  "npm run verify:s2.32", "npm run verify:s2.33",
  "tests/browser/rotary-waypoint-plan.spec.ts",
  "tests/browser/rotary-waypoint-plan-attestation.spec.ts",
  "s2-33-browser-failure"
]) if (!workflow.includes(token)) throw new Error(`S2.33 candidate CI contract missing: ${token}`);
if (workflow.includes("contents: write")) throw new Error("S2.33 CI must remain read-only");

console.log("S2.33 Rotary Waypoint Plan-Execution Attestation PASS · consumed S2.31 plan snapshot + canonical S2.30 run + S2.32 read-only execution authority + immutable session-event attestation + live Named Position edits do not rewrite history + no duplicate movement parser/no hidden clock/no dynamics solver + Tehkné Solutions");
