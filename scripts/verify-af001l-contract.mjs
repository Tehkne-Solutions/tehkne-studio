...
const [packageText, defaultPlaywright, hardwarePlaywright, hardwareSpec, workflow] = await Promise.all([
  ...
  read("playwright.af001l.config.ts"),
  read("tests/hardware/asset-forge-af001l.spec.ts"),
  read(".github/workflows/asset-forge-af001l-target-hardware.yml")
]);
...
assert.match(defaultPlaywright, /testDir:\s*"\.\/tests\/browser"/);
...
for (const token of [
  "AF001L_HARDWARE_ID",
  "AF001L_PHYSICAL_ATTESTATION",
  "AF001L_RUNNER_NAME",
  "AF001L_RUNNER_OS",
  "AF001L_RUNNER_ARCH",
  "AF001L_RUNNER_CONTEXT",
  "PHYSICAL_HARDWARE_CONFIRMED",
  "self-hosted:tehkne-af001l",
  ...
  "f6b1062238c941f81bbd5c38e154add9bb4ab56b81c06f9c45989c9604dd90c8",
  "glb-json-v1",
...
const hardwareJob = workflow.split("target-hardware:")[1] ?? "";
assert.ok(hardwareJob, "AF-001L target-hardware job is required");
assert.doesNotMatch(hardwareJob, /runs-on:\s*ubuntu-/i, "AF-001L hardware job must never run on GitHub-hosted Ubuntu");
assert.doesNotMatch(hardwareJob, /runs-on:\s*windows-/i, "AF-001L hardware job must never run on GitHub-hosted Windows");
assert.doesNotMatch(hardwareJob, /runs-on:\s*macos-/i, "AF-001L hardware job must never run on GitHub-hosted macOS");

console.log(
  "AF-001L contract PASS · physical self-hosted target-hardware gate only · " +
  "headful GPU + strict v0.6.6 fingerprint + v0.6.5 source attestation + GPU attestation + six views + <100/<150 thresholds · " +
