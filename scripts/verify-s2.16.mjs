...
for (const token of ["Mechanical Assembly", "Direct Socket Wiring", "mechanicalAssemblyExtension", "deriveMechanicalAssemblyConstraints", "mechanicalAssemblyMembers", "planMechanicalAssemblyTranslation", "MechanicalConstraintSynchronizer", "coincidentFollowerPosition", "useSpatialPortEndpoint", "physicalEndpointDeclared", "Montagem mecânica bloqueada", "data-mechanical-assemblies={mechanicalConstraints.length}", "data-driver-endpoint-source={driverEndpoint.source}", "data-follower-endpoint-source={followerEndpoint.source}", "runtime.builder.connect(sourceRef, ref)", "runtime.builder.connect(from, to)", "runtime.builder.disconnect", "inventionSpatial: runtime.spatial.document()"] ) if (!workbench.includes(token)) throw new Error(`S2.16 semantic workbench contract missing: ${token}`);
...
console.log("S2.16 Mechanical Assembly Constraints PASS · semantic connectedTo-derived coincident assembly contract + AF-001 v0.6.6 physical sockets/proxy anchors + AF-001M fail-closed evidence + atomic movement + no parallel graph · Tehkné Solutions");