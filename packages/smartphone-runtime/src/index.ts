...
export interface SmartphoneSpatialProfile {
  readonly position: readonly [number, number, number];
  readonly exploded: readonly [number, number, number];
  readonly size: readonly [number, number, number];
}

export interface SmartphoneSlotProfile {
  readonly slotId: string;
  readonly definitionId: string;
  readonly entityId: EntityId;
  readonly name: string;
  readonly teardown?: boolean;
  readonly spatial: SmartphoneSpatialProfile;
}

export interface SmartphoneConnectionProfile {
  readonly id: string;
  readonly from: readonly [slotId: string, portId: string];
  readonly to: readonly [slotId: string, portId: string];
}

export interface SmartphoneBootDependencyProfile {
  readonly slotId: string;
...
export interface SmartphoneMaterializationResult {
...
function rootEntity(profile: SmartphonePresetProfile): EngineeringEntity {
  return createEngineeringEntity({
    ...
    state: "closed",
    properties: {
      formFactor: { id: "formFactor", value: "slab-smartphone", source: "studio", confidence: 1 },
...
function bootEntity(rootId: EntityId): EngineeringEntity {
  return createEngineeringEntity({
    id: "phone.boot",
    type: "BootProcess",
...
function slotMap(profile: SmartphonePresetProfile): Map<string, SmartphoneSlotProfile> {
  return new Map(profile.slots.map((slot) => [slot.slotId, slot]));
}

function connectedPortStates(profile: SmartphonePresetProfile, slotId: string): Record<string, EngineeringPort["state"]> {
...
export function validateSmartphoneProfile(profile: SmartphonePresetProfile, registry: ComponentRegistry): string[] {
  ...
  if (profile.root.type !== "Smartphone" || !profile.root.id.trim()) errors.push("smartphone root contract is invalid");
  if (profile.slots.length === 0) errors.push("smartphone profile requires component slots");
  if (!unique(profile.slots.map((slot) => slot.slotId))) errors.push("smartphone slot IDs must be unique");
  if (!unique(profile.slots.map((slot) => slot.entityId))) errors.push("smartphone entity IDs must be unique");
  if (!unique(profile.connections.map((connection) => connection.id))) errors.push("smartphone connection IDs must be unique");

  const slots = slotMap(profile);
  for (const required of ["frame", "battery", "regulator", "soc", "memory", "storage", "display", "imu", "camera", "wireless", "usbC"]) {
    if (!slots.has(required)) errors.push(`required smartphone slot missing: ${required}`);
  }

  for (const slot of profile.slots) {
    try {
      const definition = registry.get(slot.definitionId);
      if (!definition.productFamilies.includes("smartphone")) {
        errors.push(`${slot.slotId} component ${slot.definitionId} is not declared for smartphone products`);
      }
    } catch (error) {
...
export function createSmartphoneProject(
  ...
  const root = rootEntity(profile);
  const boot = bootEntity(root.id);
  const entities: EngineeringEntity[] = [root, boot];
  const relationships: EngineeringRelationship[] = [
    { id: "phone-contains-boot", source: root.id, target: boot.id, type: "contains", metadata: { role: "boot" } }
  ];
  const slots = slotMap(profile);
  ...
  for (const dependency of profile.bootDependencies) {
    relationships.push({
      id: `phone-boot-depends-${dependency.slotId}`,
      source: boot.id,
