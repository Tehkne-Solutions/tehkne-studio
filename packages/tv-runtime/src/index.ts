...
export function validateTvProfile(profile: TvPresetProfile, registry: ComponentRegistry): string[] {
  const errors: string[] = [];
  ...
  if (profile.productFamily !== "display-system") errors.push("tv profile productFamily must be display-system");
...
export function createTvProject(profile: TvPresetProfile, registry: ComponentRegistry): ProductMaterializationResult {
