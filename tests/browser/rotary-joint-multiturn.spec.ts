...
test("S2.22 preserves multi-turn rotary history across JOINT steps, S2.21 target angles and reload", async ({ page }) => {
  ...
  await expect(joint).toHaveAttribute("data-multiturn-mode", "explicit-revolution-count");
  await expect(joint).toHaveAttribute("data-revolutions", "0");
  await expect(joint).toHaveAttribute("data-angle-unwrapped-rad", "0.000");

  const plus = joint.getByRole("button", { name: "JOINT +", exact: true });
  for (let index = 0; index < 24; index += 1) await plus.click();
  await expect(joint).toHaveAttribute("data-revolutions", "1");
  await expect(joint).toHaveAttribute("data-angle-rad", "0.000");
  await expect(joint).toHaveAttribute("data-angle-unwrapped-rad", "6.283");
  await expect(joint).toContainText("+1 REV");
  await expect(joint).toContainText("360.0°");
  ...
  await expect(joint).toHaveAttribute("data-angle-rad", "3.142");
  await expect(joint).toHaveAttribute("data-revolutions", "1");
  await expect(joint).toHaveAttribute("data-angle-unwrapped-rad", "9.425");
  ...
  await expect(joint).toHaveAttribute("data-angle-unwrapped-rad", "9.599");
  await expect(joint).toContainText("+2 REV");
  ...
  const serialized = await page.evaluate(() => window.localStorage.getItem("tehkne-studio:s2.2:project:invention"));
  expect(serialized).toContain("inventionRotaryKinematics");
  expect(serialized).toContain('"revolutions":2');
  ...
  await expect(restoredJoint).toHaveAttribute("data-revolutions", "2");
  await expect(restoredJoint).toHaveAttribute("data-angle-rad", "-2.967");
  await expect(restoredJoint).toHaveAttribute("data-angle-unwrapped-rad", "9.599");
