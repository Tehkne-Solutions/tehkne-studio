...
test("S2.21 preserves explicit multi-turn rotary kinematics through principal wrap, rigid rotation and reload", async ({ page }) => {
  ...
  for (let index = 0; index < 13; index += 1) await plus.click();
  await expect(joint).toHaveAttribute("data-revolutions", "1");
  await expect(joint).toHaveAttribute("data-angle-rad", "-2.880");
  await expect(joint).toHaveAttribute("data-angle-unwrapped-rad", "3.403");

  for (let index = 13; index < 24; index += 1) await plus.click();
  await expect(joint).toHaveAttribute("data-revolutions", "1");
  ...
  expect(serialized).toContain('"revolutions":1');
  ...
  await expect(restoredJoint).toHaveAttribute("data-revolutions", "1");
  await expect(restoredJoint).toHaveAttribute("data-angle-rad", "0.000");
  await expect(restoredJoint).toHaveAttribute("data-angle-unwrapped-rad", "6.283");
  ...
  await expect(restoredJoint).toHaveAttribute("data-angle-unwrapped-rad", "6.021");
