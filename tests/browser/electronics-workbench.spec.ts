...
function electronicsPanel(page: import("@playwright/test").Page) {
  return page.getByRole("complementary", { name: "Electronics Workbench", exact: true });
}
...
test("S2.8 Electronics Workbench teaches safe circuit, measurement, fault and persistent recovery evidence", async ({ page }) => {
  ...
  const panel = electronicsPanel(page);
...
test("S2.8 electronics natural-language controls change the same simulated circuit", async ({ page }) => {
  ...
  await expect(electronicsPanel(page).getByText("PASS", { exact: true })).toBeVisible();
