...
test("S2.24 derives continuous multi-turn angle across repeated steps targets and restore", async ({ page }) => {
  ...
  await expect(joint).toHaveAttribute("data-kinematics-source", "session-events+spatial");
  ...
  await joint.getByRole("button", { name: "SET ANGLE", exact: true }).click();
  await expect(joint).toHaveAttribute("data-angle-rad", "-2.967");
  await expect(joint).toHaveAttribute("data-continuous-angle-rad", "9.599");
  await expect(joint).toHaveAttribute("data-revolutions", "2");
  await expect(joint).toHaveAttribute("data-kinematics-evidence", "26");
  ...
  await expect(restored).toHaveAttribute("data-angle-rad", "-2.967", { timeout: 20_000 });
  await expect(restored).toHaveAttribute("data-continuous-angle-rad", "9.599");
  await expect(restored).toHaveAttribute("data-revolutions", "2");
  await expect(restored).toHaveAttribute("data-kinematics-evidence", "26");
  await restored.getByRole("button", { name: "JOINT +", exact: true }).click();
  await expect(restored).toHaveAttribute("data-command-id", "mechanical-cmd-27");
  await expect(restored).toHaveAttribute("data-continuous-angle-rad", "9.861");
