...
test("S2.7 TV 01 runs power-supply causal teardown, recovery and persistence", async ({ page }) => {
  ...
  await expect(page.getByText(/TV-01 · 8 COMPONENTES · CLOSED/)).toBeVisible();
  await expect(page.getByText(/POWER OFF · BOOT IDLE/)).toBeVisible();

  await command(page, "Abra a TV");
  await expect(page.getByText(/TV-01 · 8 COMPONENTES · OPEN/)).toBeVisible();
  ...
  await command(page, "Ligue a televisão");
...
test("S2.7 Component Library exposes canonical display extension plus reusable compatibility overlay", async ({ page }) => {
  await page.goto("/", { waitUntil: "networkidle" });
  ...
  await expect(library.getByRole("button", { name: /AC\/DC Display Power Supply/ })).toBeVisible();
  await expect(library.getByRole("button", { name: /Display Media Controller/ })).toBeVisible();
  await expect(library.getByRole("button", { name: /Large OLED Display Panel/ })).toBeVisible();
  await expect(library.getByRole("button", { name: /HDMI Input Interface/ })).toBeVisible();
  await expect(library.getByRole("button", { name: /Stereo Speaker Module/ })).toBeVisible();
  await expect(library.getByRole("button", { name: /Solid-State Storage/ })).toBeVisible();
