...
test("S2.28 derives explicit segment-average rotary rate across step targets limits HOME and restore", async ({ page }) => {
  const errors: string[] = [];
  ...
  await expect(joint).toHaveAttribute("data-home-rad", "7.592");
  await expect(joint).toHaveAttribute("data-home-command-id", "mechanical-home-cmd-1");
  await expect(joint).toHaveAttribute("data-rate-command-id", "mechanical-cmd-4");

  await continuous.fill("255");
  await duration.fill("6");
  await joint.getByRole("button", { name: "SET CONTINUOUS", exact: true }).click();
  await expect(joint).toHaveAttribute("data-command-id", "mechanical-cmd-6");
  await expect(joint).toHaveAttribute("data-continuous-angle-rad", "4.451");
  await expect(joint).toHaveAttribute("data-rate-command-id", "mechanical-cmd-6");
  await expect(joint).toHaveAttribute("data-rpm", "-5.000");

  await duration.fill("6");
  await joint.getByRole("button", { name: "GO HOME", exact: true }).click();
  await expect(joint).toHaveAttribute("data-command-id", "mechanical-cmd-7");
  await expect(joint).toHaveAttribute("data-command-mode", "continuous-absolute");
  await expect(joint).toHaveAttribute("data-continuous-angle-rad", "7.592");
  await expect(joint).toHaveAttribute("data-rate-command-id", "mechanical-cmd-7");
  await expect(joint).toHaveAttribute("data-duration-seconds", "6.000");
  await expect(joint).toHaveAttribute("data-angular-velocity-rad-s", "0.524");
  await expect(joint).toHaveAttribute("data-rpm", "5.000");
  await expect(joint).toContainText("HOME +7.592 rad");
  ...
  await expect(restored).toHaveAttribute("data-rate-command-id", "mechanical-cmd-7", { timeout: 20_000 });
  await expect(restored).toHaveAttribute("data-rate-mode", "segment-average");
  await expect(restored).toHaveAttribute("data-duration-seconds", "6.000");
  await expect(restored).toHaveAttribute("data-rpm", "5.000");
  await expect(restored).toHaveAttribute("data-travel-limited", "true");
  await expect(restored).toHaveAttribute("data-home-authored", "true");
  await expect(restored).toHaveAttribute("data-home-rad", "7.592");

  await restored.getByRole("button", { name: "JOINT +", exact: true }).click();
  await expect(restored).toHaveAttribute("data-command-id", "mechanical-cmd-8");
  await expect(restored).toHaveAttribute("data-rate-command-id", "mechanical-cmd-8");
