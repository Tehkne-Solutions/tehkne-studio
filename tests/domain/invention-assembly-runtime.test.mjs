...
function assertClose(actual, expected, epsilon = 1e-12) { assert.ok(Math.abs(actual - expected) <= epsilon, `expected ${actual} to be within ${epsilon} of ${expected}`); }

test("S2.16 derives a coincident mechanical constraint from the authoritative connectedTo relationship", async () => {
  ...
  spatial.ensureComponent(motor.id); spatial.ensureComponent(wheel.id);
  ...
  assert.equal(constraints.length, 1); assert.equal(constraints[0].relationshipId, connection.id);
  assert.deepEqual(constraints[0].driver, { entityId: motor.id, portId: "shaft-out" }); assert.deepEqual(constraints[0].follower, { entityId: wheel.id, portId: "hub-in" });
  assert.deepEqual(constraints[0].sharedInterfaces, ["mechanical.rotary-shaft"]); assert.equal(constraints[0].constraint, "coincident"); assert.equal(constraints[0].derivedFrom, "engineering-graph");
  assert.deepEqual(new Set(mechanicalAssemblyMembers(constraints, motor.id)), new Set([motor.id, wheel.id]));
});

test("S2.16 coincident snap translates only the follower binding and closes the physical endpoint gap", async () => {
  const { builder, spatial } = await assemblyRuntime();
  const motor = builder.addComponent("actuation.motor.dc-brushed-v1"); const wheel = builder.addComponent("mechanical.wheel.drive-v1");
  const motorBinding = spatial.ensureComponent(motor.id, { x: -0.2, y: 0.1, z: 0 }); const wheelBinding = spatial.ensureComponent(wheel.id, { x: 0.2, y: 0.1, z: 0 });
  const driverEndpoint = { x: motorBinding.position.x, y: motorBinding.position.y, z: motorBinding.position.z + 0.03185 }; const followerEndpoint = { ...wheelBinding.position };
  const next = coincidentFollowerPosition(driverEndpoint, followerEndpoint, wheelBinding); assert.deepEqual(next, driverEndpoint);
  const moved = spatial.move(wheel.id, next); assert.deepEqual(moved.position, driverEndpoint); assert.equal(endpointsAreCoincident(driverEndpoint, moved.position), true);
  assert.deepEqual(spatial.binding(motor.id).position, motorBinding.position, "snap must not move the driver");
});

test("S2.16 assembly translation is atomic and fails closed before any member leaves spatial bounds", async () => {
  const { session, builder, spatial } = await assemblyRuntime();
  const motor = builder.addComponent("actuation.motor.dc-brushed-v1"); const wheel = builder.addComponent("mechanical.wheel.drive-v1");
  spatial.ensureComponent(motor.id, { x: 0.35, y: 0, z: 0 }); spatial.ensureComponent(wheel.id, { x: 0.38, y: 0, z: 0 });
  builder.connect({ entityId: motor.id, portId: "shaft-out" }, { entityId: wheel.id, portId: "hub-in" });
  const constraints = deriveMechanicalAssemblyConstraints(session, builder.connections()); const members = mechanicalAssemblyMembers(constraints, wheel.id); const before = spatial.bindings();
  assert.throws(() => planMechanicalAssemblyTranslation(before, members, { x: 0.2, y: 0, z: 0 }), /outside invention workspace bounds/);
  assert.deepEqual(spatial.bindings(), before, "planning failure must not partially mutate the spatial scene");
  const plan = planMechanicalAssemblyTranslation(before, members, { x: 0.05, y: 0, z: 0 }); for (const move of plan) spatial.move(move.entityId, move.to);
  assertClose(spatial.binding(motor.id).position.x, 0.4); assertClose(spatial.binding(wheel.id).position.x, 0.43);
});

test("S2.16 keeps electrical relationships out of the mechanical constraint projection", async () => {
  const { session, builder, spatial } = await assemblyRuntime();
