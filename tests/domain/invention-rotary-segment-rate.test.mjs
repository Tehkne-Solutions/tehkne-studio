...
test("S2.28 derives deterministic segment-average angular rate and RPM only from explicit duration", () => {
...
test("S2.28 untimed rotary commands keep rate unresolved instead of inferring from wall-clock timestamps", async () => {
...
test("S2.28 timed step records and restores segment-average rate through the existing session event stream", async () => {
...
test("S2.28 timed principal target uses the actual shortest delta and explicit duration for segment rate", async () => {
...
test("S2.28 timed continuous target uses the exact multi-turn delta instead of the principal representative", async () => {
...
test("S2.28 latest untimed motion makes rate unresolved while travel and HOME authoring do not replace motion evidence", async () => {
  ...
  assert.equal(commands.rate(connection.id).mode, "segment-average", "SET HOME is not rotary motion");
  assert.equal((await home.clearHome(connection.id, "ui")).ok, true);
  assert.equal(commands.rate(connection.id).mode, "segment-average", "CLEAR HOME is not rotary motion");
...
test("S2.28 travel-limit rejection is fail closed and cannot publish false timed rate evidence", async () => {
...
test("S2.28 GO HOME with explicit duration derives rate from the canonical continuous movement event", async () => {
  const { session, spatial, connection } = await rotaryRuntime("segment-rate-home");
  const commands = mechanicalCommandRuntimeFor(spatial);
  const home = mechanicalRotaryHomeRuntimeFor(spatial);
  ...
  const returned = await home.goHome(connection.id, "automation", 6);
  assert.equal(returned.ok, true, returned.error);
  assert.ok(returned.result);
  assert.equal(returned.result.mode, "continuous-absolute");
  ...
  assert.equal(rate.mode, "segment-average");
  assert.equal(rate.commandId, returned.result.commandId, "rate must be sourced from the canonical movement command, not the HOME orchestration command");
  close(rate.averageRpm, 5);
  assert.equal(session.events.list().length, eventsBefore + 2, "GO HOME should add one movement event and one request audit event");
  const request = session.events.list().at(-1);
  assert.equal(request?.type, "MechanicalRotaryHomeRequested");
...
test("S2.28 rejects tampered persisted rate evidence", async () => {
