...
function reconstructCompressedPayload(): Buffer {
  ...
  if (payload.byteLength !== SOURCE_COMPRESSED_BYTES) {
    throw new Error(
      `AF001 v0.6.6 source Brotli byte-length mismatch: expected ${SOURCE_COMPRESSED_BYTES}, received ${payload.byteLength}`
    );
  }
  ...
  if (compressedDigest !== SOURCE_COMPRESSED_SHA256) {
    throw new Error(
      `AF001 v0.6.6 source Brotli SHA-256 mismatch: expected ${SOURCE_COMPRESSED_SHA256}, received ${compressedDigest}`
    );
  }
...
function materializeSourceMotor(): Buffer {
  ...
  try {
    ...
    throw new Error(
      `AF001 v0.6.6 source Brotli decompression failed: ${error instanceof Error ? error.message : String(error)}`
    );
  }

  if (motor.byteLength !== SOURCE_BYTES) {
    throw new Error(
      `AF001 v0.6.6 source GLB byte-length mismatch: expected ${SOURCE_BYTES}, received ${motor.byteLength}`
    );
  }
  ...
  if (motorDigest !== SOURCE_SHA256) {
    throw new Error(
      `AF001 v0.6.6 source GLB SHA-256 mismatch: expected ${SOURCE_SHA256}, received ${motorDigest}`
    );
  }
...
function patchSocketTransforms(source: Buffer): Buffer {
  ...
  if (source.readUInt32LE(0) !== GLB_MAGIC) throw new Error("AF001 v0.6.6 source GLB magic mismatch");
  if (source.readUInt32LE(4) !== GLB_VERSION) throw new Error("AF001 v0.6.6 source GLB version mismatch");
  if (source.readUInt32LE(8) !== source.byteLength) throw new Error("AF001 v0.6.6 source GLB total length mismatch");
  ...
  const document = JSON.parse(source.subarray(jsonStart, jsonEnd).toString("utf8").trimEnd()) as {
  ...
  if (patchedCount !== 4) throw new Error(`AF001 v0.6.6 socket patch count mismatch: ${patchedCount}`);
...
function materializeMotor(): Buffer {
  ...
  if (motor.byteLength !== EXPECTED_BYTES) {
    throw new Error(
      `AF001 v0.6.6 GLB byte-length mismatch: expected ${EXPECTED_BYTES}, received ${motor.byteLength}`
    );
  }
  const motorDigest = digest(motor);
  if (motorDigest !== EXPECTED_SHA256) {
    throw new Error(
      `AF001 v0.6.6 GLB SHA-256 mismatch: expected ${EXPECTED_SHA256}, received ${motorDigest}`
    );
  }
  return motor;
}
