...
import { MechanicalOrientationSynchronizer } from "./MechanicalOrientationSynchronizer";
...
export function Invention3DWorkbench() {
  ...
  const orientationConstraints = useMemo(() => deriveMechanicalOrientationConstraints(runtime.session, connections), [connections, runtime, revision]);
  ...
  const rotateSelected = (axis: keyof SpatialVector3, delta: number): void => {
    if (!selectedEntityId || !selectedBinding) return;
    try {
      const next = { ...selectedBinding.rotation, [axis]: selectedBinding.rotation[axis] + delta };
      const rotated = runtime.spatial.rotate(selectedEntityId, next);
      changed(`Orientação 3D · ${selectedEntity?.name ?? selectedEntityId} · rx ${format(rotated.rotation.x)} · ry ${format(rotated.rotation.y)} · rz ${format(rotated.rotation.z)} · axis constraints ${orientationConstraints.length}.`);
    } catch (cause) { blocked(cause); }
  };
  ...
  const save = (): void => { try { saveBrowserProject("invention", createSessionSnapshot(runtime.session, { extensions: { invention: runtime.builder.document(), inventionSpatial: runtime.spatial.document() } })); setSaved(true); changed(`Bancada 3D salva · ${components.length} componentes · ${connections.length} conexões · ${bindings.length} bindings · ${mechanicalConstraints.length} constraints coincident · ${orientationConstraints.length} axis-aligned · sem simulação implícita.`); } catch (cause) { blocked(cause); } };
  ...
  const orientationAligned = (constraint: MechanicalOrientationConstraint): void => { setMessage(`Orientação ${constraint.relationshipId} alinhada ao eixo físico · ${constraint.sharedInterfaces.join(", ")} · mesma relação connectedTo.`); setError(false); setRevision((current) => current + 1); };
  const orientationBlocked = (constraint: MechanicalOrientationConstraint, cause: unknown): void => { try { runtime.builder.disconnect(constraint.relationshipId); } catch {} setError(true); setMessage(cause instanceof Error ? `Orientação ${constraint.relationshipId} bloqueada: ${cause.message}` : `Orientação ${constraint.relationshipId} bloqueada.`); setRevision((current) => current + 1); };
    ...
    {open ? <section className={styles.overlay} aria-label="3D Invention Workbench"><div className={styles.shell}>
      ...
      <div className={styles.body}>
        ...
        <aside className={styles.inspector}><span>TRANSFORM 3D</span>{selectedEntity && selectedBinding ? <><div className={styles.selected} data-testid="invention-3d-selected" data-x={format(selectedBinding.position.x)} data-y={format(selectedBinding.position.y)} data-z={format(selectedBinding.position.z)} data-rx={format(selectedBinding.rotation.x)} data-ry={format(selectedBinding.rotation.y)} data-rz={format(selectedBinding.rotation.z)}><strong>{selectedEntity.name}</strong><small>x {format(selectedBinding.position.x)} · y {format(selectedBinding.position.y)} · z {format(selectedBinding.position.z)} · rx {format(selectedBinding.rotation.x)} · ry {format(selectedBinding.rotation.y)} · rz {format(selectedBinding.rotation.z)}</small></div><div className={styles.selected} data-testid="invention-3d-visual-source" data-source={selectedVisual ? "asset" : "proxy"} data-asset-id={selectedVisual?.assetId ?? ""} data-asset-version={selectedVisual?.version ?? ""} data-asset-lod={selectedVisual?.lod ?? ""} data-socket-count={selectedVisual ? Object.keys(selectedVisual.portSocketMap).length : 0} data-sockets={selectedVisual ? Object.values(selectedVisual.portSocketMap).join(",") : ""} data-proxy-kind={selectedProxy?.kind ?? ""} data-anchor-count={selectedProxy ? Object.keys(selectedProxy.portAnchors).length : 0} data-anchors={selectedProxy ? Object.values(selectedProxy.portAnchors).map((anchor) => anchor.name).join(",") : ""}><strong>{selectedVisual ? "REAL ASSET" : "PROXY EXPLÍCITO"}</strong><small>{selectedVisual ? `${selectedVisual.assetId} · ${selectedVisual.version} · ${selectedVisual.lod} · ${Object.keys(selectedVisual.portSocketMap).length} SOCKETS` : selectedProxy ? `${selectedProxy.kind.toUpperCase()} · ${Object.keys(selectedProxy.portAnchors).length} ÂNCORA(S) FÍSICA(S)` : "Ainda sem arte Asset Forge cadastrada."}</small></div>
          ...
          <div className={styles.axisGrid}><button type="button" onClick={() => moveSelected("x", -MOVE_STEP)}>X −</button><button type="button" onClick={() => moveSelected("x", MOVE_STEP)}>X +</button><button type="button" onClick={() => moveSelected("y", -MOVE_STEP)}>Y −</button><button type="button" onClick={() => moveSelected("y", MOVE_STEP)}>Y +</button><button type="button" onClick={() => moveSelected("z", -MOVE_STEP)}>Z −</button><button type="button" onClick={() => moveSelected("z", MOVE_STEP)}>Z +</button></div><div className={styles.axisGrid} data-testid="invention-3d-rotation-controls"><button type="button" onClick={() => rotateSelected("x", -ROTATE_STEP)}>RX −</button><button type="button" onClick={() => rotateSelected("x", ROTATE_STEP)}>RX +</button><button type="button" onClick={() => rotateSelected("y", -ROTATE_STEP)}>RY −</button><button type="button" onClick={() => rotateSelected("y", ROTATE_STEP)}>RY +</button><button type="button" onClick={() => rotateSelected("z", -ROTATE_STEP)}>RZ −</button><button type="button" onClick={() => rotateSelected("z", ROTATE_STEP)}>RZ +</button></div></> : <p>Selecione uma entidade na lista ou diretamente na viewport.</p>}
          ...
          <div className={styles.wireEvidence} aria-label="Mechanical orientation constraints">{orientationConstraints.map((constraint) => { const driverBinding = bindingMap.get(constraint.driver.entityId); const followerBinding = bindingMap.get(constraint.follower.entityId); if (!driverBinding || !followerBinding) return null; return <MechanicalOrientationSynchronizer key={constraint.relationshipId} constraint={constraint} driverBinding={driverBinding} followerBinding={followerBinding} spatial={runtime.spatial} onAligned={orientationAligned} onBlocked={orientationBlocked} />; })}</div>
        </aside>
