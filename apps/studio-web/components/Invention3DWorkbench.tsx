...
import { RotaryJointControls } from "./RotaryJointControls";
...
function formatAxis(value: SpatialVector3): string { return `${value.x.toFixed(4)},${value.y.toFixed(4)},${value.z.toFixed(4)}`; }
...
function MechanicalConstraintSynchronizer({ constraint, axialConstraint, sourceEntity, targetEntity, sourceBinding, targetBinding, spatial, onSnapped, onBlocked }: {
  readonly constraint: MechanicalAssemblyConstraint; readonly axialConstraint?: MechanicalAxialConstraint; readonly sourceEntity: EngineeringEntity; readonly targetEntity: EngineeringEntity; readonly sourceBinding: SpatialEntityBinding; readonly targetBinding: SpatialEntityBinding; readonly spatial: InventionSpatialScene;
  ...
  const driverAxis = axialConstraint ? mechanicalWorldAxis(axialConstraint.driverAxisLocal, sourceBinding.rotation) : null;
  const followerAxis = axialConstraint ? mechanicalWorldAxis(axialConstraint.followerAxisLocal, targetBinding.rotation) : null;
  const axisAligned = !axialConstraint || Boolean(driverAxis && followerAxis && mechanicalAxesAreAligned(driverAxis, followerAxis));
  const satisfied = resolved && coincident && axisAligned;
  useEffect(() => {
    if (!resolved || satisfied) return;
    try {
      if (axialConstraint) {
        const plan = planMechanicalAxialAlignment(
          driverEndpoint.position,
          followerEndpoint.localPosition,
          axialConstraint.driverAxisLocal,
          axialConstraint.followerAxisLocal,
          sourceBinding,
          targetBinding
        );
  ...
  }, [axialConstraint, constraint.follower.entityId, constraint.relationshipId, driverEndpoint.localPosition.x, driverEndpoint.localPosition.y, driverEndpoint.localPosition.z, driverEndpoint.position.x, driverEndpoint.position.y, driverEndpoint.position.z, followerEndpoint.localPosition.x, followerEndpoint.localPosition.y, followerEndpoint.localPosition.z, followerEndpoint.position.x, followerEndpoint.position.y, followerEndpoint.position.z, resolved, satisfied, sourceBinding.rotation.x, sourceBinding.rotation.y, sourceBinding.rotation.z, spatial, targetBinding.position.x, targetBinding.position.y, targetBinding.position.z, targetBinding.rotation.x, targetBinding.rotation.y, targetBinding.rotation.z]);
  const state = !resolved ? "waiting" : satisfied ? "snapped" : "aligning";
  const axialState = !axialConstraint ? "not-applicable" : satisfied ? "aligned" : "aligning";
  return <div data-testid={`mechanical-constraint-${constraint.relationshipId}`} data-state={state} data-axial-state={axialState} data-rotary-dof={axialConstraint ? "enabled" : "none"} data-derived-from={constraint.derivedFrom} data-driver-entity={constraint.driver.entityId} data-driver-port={constraint.driver.portId} data-driver-endpoint={driverEndpoint.socketName} data-driver-endpoint-source={driverEndpoint.source} data-follower-entity={constraint.follower.entityId} data-follower-port={constraint.follower.portId} data-follower-endpoint={followerEndpoint.socketName} data-follower-endpoint-source={followerEndpoint.source} data-driver-axis={driverAxis ? formatAxis(driverAxis) : ""} data-follower-axis={followerAxis ? formatAxis(followerAxis) : ""} data-driver-x={format(driverEndpoint.position.x)} data-driver-y={format(driverEndpoint.position.y)} data-driver-z={format(driverEndpoint.position.z)} data-follower-x={format(followerEndpoint.position.x)} data-follower-y={format(followerEndpoint.position.y)} data-follower-z={format(followerEndpoint.position.z)}><strong>ASSEMBLY · {constraint.sharedInterfaces.join(" · ")}</strong><small>{constraint.driver.portId} → {constraint.follower.portId} · {resolved ? (satisfied ? (axialConstraint ? "SNAPPED · AXIS ALIGNED · ROTARY DOF" : "SNAPPED") : "ALIGNING") : "WAITING FOR PHYSICAL ENDPOINT"}</small></div>;
}
...
export function Invention3DWorkbench() {
  ...
  const axialConstraints = useMemo(() => deriveMechanicalAxialConstraints(runtime.session, connections), [connections, runtime, revision]);
  const axialConstraintMap = useMemo(() => new Map(axialConstraints.map((constraint) => [constraint.relationshipId, constraint])), [axialConstraints]);
  ...
  const save = (): void => { try { saveBrowserProject("invention", createSessionSnapshot(runtime.session, { extensions: { invention: runtime.builder.document(), inventionSpatial: runtime.spatial.document() } })); setSaved(true); changed(`Bancada 3D salva · ${components.length} componentes · ${connections.length} conexões · ${bindings.length} bindings · ${mechanicalConstraints.length} constraints · ${axialConstraints.length} rotary joint DOF(s) · sem simulação implícita.`); } catch (cause) { blocked(cause); } };
  ...
  const assemblySnapped = (constraint: MechanicalAssemblyConstraint): void => { const axial = axialConstraintMap.has(constraint.relationshipId); setMessage(`Assembly ${constraint.relationshipId} ${axial ? "encaixado e axialmente alinhado" : "encaixado"} por endpoints físicos · ${constraint.sharedInterfaces.join(", ")} · constraint derivada do Engineering Graph.`); setError(false); setRevision((current) => current + 1); };
    ...
    {open ? <section className={styles.overlay} aria-label="3D Invention Workbench"><div className={styles.shell}>
      ...
      <div className={styles.body}>
        ...
        <aside className={styles.inspector}><span>TRANSFORM 3D</span>{selectedEntity && selectedBinding ? <><div className={styles.selected} data-testid="invention-3d-selected" data-x={format(selectedBinding.position.x)} data-y={format(selectedBinding.position.y)} data-z={format(selectedBinding.position.z)} data-rx={format(selectedBinding.rotation.x)} data-ry={format(selectedBinding.rotation.y)} data-rz={format(selectedBinding.rotation.z)}><strong>{selectedEntity.name}</strong><small>x {format(selectedBinding.position.x)} · y {format(selectedBinding.position.y)} · z {format(selectedBinding.position.z)} · rx {format(selectedBinding.rotation.x)} · ry {format(selectedBinding.rotation.y)} · rz {format(selectedBinding.rotation.z)}</small></div><div className={styles.selected} data-testid="invention-3d-visual-source" data-source={selectedVisual ? "asset" : "proxy"} data-asset-id={selectedVisual?.assetId ?? ""} data-asset-version={selectedVisual?.version ?? ""} data-asset-lod={selectedVisual?.lod ?? ""} data-socket-count={selectedVisual ? Object.keys(selectedVisual.portSocketMap).length : 0} data-sockets={selectedVisual ? Object.values(selectedVisual.portSocketMap).join(",") : ""} data-proxy-kind={selectedProxy?.kind ?? ""} data-anchor-count={selectedProxy ? Object.keys(selectedProxy.portAnchors).length : 0} data-anchors={selectedProxy ? Object.values(selectedProxy.portAnchors).map((anchor) => anchor.name).join(",") : ""}><strong>{selectedVisual ? "REAL ASSET" : "PROXY EXPLÍCITO"}</strong><small>{selectedVisual ? `${selectedVisual.assetId} · ${selectedVisual.version} · ${selectedVisual.lod} · ${Object.keys(selectedVisual.portSocketMap).length} SOCKETS` : selectedProxy ? `${selectedProxy.kind.toUpperCase()} · ${Object.keys(selectedProxy.portAnchors).length} ÂNCORA(S) FÍSICA(S)` : "Ainda sem arte Asset Forge cadastrada."}</small></div>
          ...
          <div className={styles.wireEvidence} aria-label="Mechanical assembly constraints">{mechanicalConstraints.map((constraint) => { const sourceEntity = componentMap.get(constraint.driver.entityId); const targetEntity = componentMap.get(constraint.follower.entityId); const sourceBinding = bindingMap.get(constraint.driver.entityId); const targetBinding = bindingMap.get(constraint.follower.entityId); const axialConstraint = axialConstraintMap.get(constraint.relationshipId); if (!sourceEntity || !targetEntity || !sourceBinding || !targetBinding) return null; return <div key={constraint.relationshipId}><MechanicalConstraintSynchronizer constraint={constraint} axialConstraint={axialConstraint} sourceEntity={sourceEntity} targetEntity={targetEntity} sourceBinding={sourceBinding} targetBinding={targetBinding} spatial={runtime.spatial} onSnapped={assemblySnapped} onBlocked={assemblyBlocked} />{axialConstraint ? <RotaryJointControls constraint={axialConstraint} driverEntity={sourceEntity} followerEntity={targetEntity} driverBinding={sourceBinding} followerBinding={targetBinding} spatial={runtime.spatial} onChanged={(radians) => changed(`Joint Rotate 3D · ${targetEntity.name} · ${radians >= 0 ? "+" : "−"}${format(Math.abs(radians))} rad · driver ${sourceEntity.name} imóvel · mesma constraint ${constraint.relationshipId}.`)} onBlocked={blocked} /> : null}</div>; })}</div>
        </aside>
