...
export function Invention3DWorkbench() {
  ...
  const save = (): void => { try { saveBrowserProject("invention", createSessionSnapshot(runtime.session, { extensions: { invention: runtime.builder.document(), inventionSpatial: runtime.spatial.document() } })); setSaved(true); changed(`Bancada 3D salva · ${components.length} componentes · ${connections.length} conexões · ${bindings.length} bindings · ${mechanicalConstraints.length} constraints · ${axialConstraints.length} axial joint(s) · sem simulação implícita.`); } catch (cause) { blocked(cause); } };
