...
function materialKey(material: Material, geometry: BufferGeometry): string {
  return `${material.uuid}::${geometrySignature(geometry)}`;
}
...
function batchStaticMeshes(root: Object3D): AssetInspection {
  ...
  root.traverse((object) => {
    ...
    const key = materialKey(object.material, geometry);
  ...
  runtimeGroup.name = "AF001_RUNTIME_BATCHES";
  ...
  const visibleRenderMeshes = renderMeshes.filter((mesh) => mesh.visible);
  const shadowCasters = [...visibleRenderMeshes]
    .sort((a, b) => geometryTriangleCount(b.geometry) - geometryTriangleCount(a.geometry))
    .slice(0, MAX_SHADOW_CASTERS);
  for (const mesh of shadowCasters) mesh.castShadow = true;

  return {
    missingNodes: REQUIRED_NODES.filter((name) => !root.getObjectByName(name)),
    sourceMeshCount: sourceMeshes.length,
    renderMeshCount: visibleRenderMeshes.length,
...
export function GoldenMotorPbrReviewGateV065RuntimeBatched() {
    ...
    >
      <header style={{ display: "flex", justifyContent: "space-between", gap: 24, alignItems: "end", flexWrap: "wrap" }}>
        <div>
          ...
          <h1 style={{ margin: "8px 0 0", fontSize: "clamp(26px, 4vw, 42px)", letterSpacing: "-.03em" }}>AF-001I · HERO v0.6.5 Runtime Review</h1>
          <p style={{ color: "#9da7ae", margin: "8px 0 0", maxWidth: 820, lineHeight: 1.55 }}>
            Golden Motor HERO_CANDIDATE v0.6.5 · 3.292 tris · runtime batching por material · PBR e topologia autorada preservados.
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(300px, 360px)", gap: 16, minHeight: 0 }}>
        <div data-testid="pbr-canvas-shell" data-camera-view={view} style={{ minHeight: 600, borderRadius: 18, overflow: "hidden", border: "1px solid #343d43", background: "#151a1e" }}>
          ...
          >
            ...
            <directionalLight
              ...
              shadow-camera-left={-0.055}
              shadow-camera-right={0.055}
              shadow-camera-top={0.055}
              shadow-camera-bottom={-0.055}
              shadow-camera-near={0.01}
              shadow-camera-far={0.30}
              shadow-bias={-0.00008}
            />
        ...
        <aside style={{ border: "1px solid #343d43", borderRadius: 18, padding: 20, background: "#11161a" }}>
          ...
          <dl style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "10px 16px", margin: 0 }}>
            ...
            <dt>Meshes source</dt><dd data-testid="mesh-count" style={{ margin: 0, fontWeight: 800 }}>{inspection?.sourceMeshCount ?? "—"}</dd>
            <dt>Meshes render</dt><dd data-testid="render-mesh-count" style={{ margin: 0, fontWeight: 800 }}>{inspection?.renderMeshCount ?? "—"}</dd>
            <dt>Materiais</dt><dd data-testid="material-count" style={{ margin: 0, fontWeight: 800 }}>{inspection?.materialCount ?? "—"}</dd>
            <dt>Shadow casters</dt><dd data-testid="shadow-caster-count" style={{ margin: 0, fontWeight: 800 }}>{inspection?.shadowCasterCount ?? "—"}</dd>
          ...
          <p style={{ color: "#7f8a92", fontSize: 12, lineHeight: 1.55 }}>
            PASS exige ≥{MIN_BENCHMARK_SAMPLES} frames, média &lt; {MAX_AVERAGE_FRAME_MS} ms e P95 &lt; {MAX_P95_FRAME_MS} ms. O batching é uma otimização de apresentação e não altera o GLB HERO autoritativo.
          </p>
        </aside>
      </div>

      <footer style={{ display: "flex", justifyContent: "space-between", color: "#78848b", fontSize: 12 }}>
        <span>HERO_CANDIDATE · static batching · fail-closed</span>
