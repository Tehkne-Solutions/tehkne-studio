...
function inspectAndTuneScene(root: Object3D): AssetInspection {
  const meshes: Mesh[] = [];
  ...
  // Shadow cost is bounded by the six silhouette-dominant meshes rather than
  // multiplying the shadow pass across every washer, rivet and stamped detail.
  // This changes only the review renderer; HERO geometry/material data remains untouched.
...
function GoldenMotor({ onReady }: { readonly onReady: (inspection: AssetInspection) => void }) {
  ...
  useEffect(() => {
    onReady(inspectAndTuneScene(scene));
  }, [onReady, scene]);
...
export function GoldenMotorPbrReviewGateV065RuntimeOptimized() {
  ...
  return (
    <section
      ...
      data-shadow-casters={inspection?.shadowCasterCount ?? 0}
      ...
      <header style={{ display: "flex", justifyContent: "space-between", gap: 24, alignItems: "end", flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ color: "#9da7ae", margin: "8px 0 0", maxWidth: 820, lineHeight: 1.55 }}>
            Golden Motor HERO_CANDIDATE v0.6.5 · LOD0 real de 3.292 tris · PBR preservado · benchmark determinístico fail-closed.
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(300px, 360px)", gap: 16, minHeight: 0 }}>
        ...
        <aside style={{ border: "1px solid #343d43", borderRadius: 18, padding: 20, background: "#11161a" }}>
          ...
          <p style={{ color: "#7f8a92", fontSize: 12, lineHeight: 1.55 }}>
            Benchmark pronto significa coleta encerrada. PASS exige ≥{MIN_BENCHMARK_SAMPLES} frames válidos, média &lt; {MAX_AVERAGE_FRAME_MS} ms e P95 &lt; {MAX_P95_FRAME_MS} ms. A arte HERO não é alterada por esta otimização de renderer.
          </p>
        </aside>
      </div>

      <footer style={{ display: "flex", justifyContent: "space-between", color: "#78848b", fontSize: 12 }}>
        <span>HERO_CANDIDATE · runtime-cost isolated · fail-closed</span>
